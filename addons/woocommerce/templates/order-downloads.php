...
<section class="ast-woo-grid-orders-container">
	...
	if ( isset( $args['show_title'] ) ) :
		$my_acccount_download_title = astra_get_i18n_option( 'my-account-download-text', _x( '%astra%', 'Download Text', 'astra-addon' ) );
		?>
			...
			<?php foreach ( wc_get_account_downloads_columns() as $column_id => $column_name ) : ?>
				<div class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
					...
					if ( has_action( 'woocommerce_account_downloads_column_' . $column_id ) ) {
						...
						$download_remaining_text  = astra_get_i18n_option( 'my-account-download-remaining-text', _x( '%astra%', 'Download Remaining Text', 'astra-addon' ) ) . ' ';
						$download_expire_text     = astra_get_i18n_option( 'my-account-download-expire-text', _x( '%astra%', 'Download Expire Text', 'astra-addon' ) ) . ' ';
						$download_expire_alt_text = astra_get_i18n_option( 'my-account-download-expire-alt-text', _x( '%astra%', 'Download Expire Alt Text', 'astra-addon' ) ) . ' ';
